...
import 'package:flutter_markdown/src/builder.dart';
...
/// Signature for custom image builders used by
/// [MarkdownWidget.sizedImageBuilder].
///
...
typedef MarkdownImageBuilder =
    Widget Function(Uri uri, String? title, String? alt);
...
typedef MarkdownBulletBuilder =
    Widget Function(MarkdownBulletParameters parameters);
...
abstract class MarkdownElementBuilder {
  ...
  Widget? visitElementAfterWithContext(
    ...
    return visitElementAfterWithContext(
      context,
      element,
      preferredStyle,
      parentStyle,
...
abstract class MarkdownWidget extends StatefulWidget {
  ...
  /// Collection of custom block syntax types to be used parsing the
  /// Markdown data.
  final List<md.BlockSyntax>? blockSyntaxes;

  /// Collection of custom inline syntax types to be used parsing the
  /// Markdown data.
...
class _MarkdownWidgetState extends State<MarkdownWidget>
  ...
  void _parseMarkdown() {
    final fallbackStyleSheet = kFallbackStyle(
      context,
      widget.styleSheetTheme,
    );
    final styleSheet = fallbackStyleSheet.merge(
      widget.styleSheet,
    );
  ...
  TextSpan formatText(MarkdownStyleSheet styleSheet, String code) {
    final replacedCode = code.replaceAll(RegExp(r'\n$'), '');
    if (widget.syntaxHighlighter != null) {
      return widget.syntaxHighlighter!.format(replacedCode);
    }
    return TextSpan(style: styleSheet.code, text: replacedCode);
  }
...
///
/// This class is no longer used as Markdown now supports checkbox syntax
/// natively.
@Deprecated(
