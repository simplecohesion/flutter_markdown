...
class SubscriptBuilder extends MarkdownElementBuilder {
  ...
  Widget visitElementAfter(md.Element element, _) {
    ...
    for (int i = 0; i < textContent.length; i++) {
      text.write(_subscripts[int.parse(textContent[i])]);
    }
    return Text.rich(TextSpan(text: text.toString()));
  }
}
...
class WikilinkSyntax extends md.InlineSyntax {
  ...
  bool onMatch(md.InlineParser parser, Match match) {
    ...
    final md.Element el = md.Element('wikilink', <md.Element>[
      md.Element.text('span', link),
...
class CustomTagBlockSyntax extends md.BlockSyntax {
  ...
  md.Node parse(md.BlockParser parser) {
    ...
    while (!parser.current.content.startsWith('{{/custom}}') &&
        !parser.isDone) {
